...
<form action="{{ route('carreras.update', $carrera) }}" method="POST" class="bg-white p-6 rounded shadow max-w-md">
    @csrf
    @method('PUT')

    <div class="mb-4">
        <label for="nombre" class="block font-semibold mb-1">Nombre de la carrera:</label>
        <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $carrera->nombre) }}" class="w-full border-gray-300 rounded p-2">
        @error('nombre')
